...
Page({
  ...
  onLoad: function(options) {
    ...
    wx.request({
      method: 'POST',
      url: app.globalData.host + '/api/pet/common/getAppoTimes',
      data: {},
      header: {
      ...
      success(res) {
        var data = res.data;
        var deletypes = [];
        deletypes.push('请选择服务项目');
        for (var i = 0; i < data.data.serv_list.length; i++) { 
            deletypes.push(data.data.serv_list[i].name + '（' + data.data.serv_list[i].price+'元）'); 
        }
        var defauult = {
          id: 0,
          name: '请选择服务项目',
  ...
  appointNow: function(e) {
    ...
    wx.request({
      method: 'POST',
      url: app.globalData.host + '/api/pet/common/appointNow',
      data: info,
      header: {
        "Content-Type": "application/x-www-form-urlencoded"
      },
